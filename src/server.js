...
// Keep this dashboard bound to localhost. It controls a logged-in browser
// session and must not be exposed through public interfaces or tunnels.
const HOST = '127.0.0.1';
const rootDir = process.cwd();
const webDir = path.join(rootDir, 'web');
const envPath = path.join(rootDir, '.env');
const cleanupCategories = KNOWN_CLEANUP_CATEGORIES;
...
async function readJson(req) {
  ...
  for await (const chunk of req) {
    ...
    if (total > MAX_JSON_BYTES) {
      throw Object.assign(new Error('Request body too large.'), { statusCode: 400 });
    }
    chunks.push(chunk);
  }
...
function startRun({ mode, maxUnfollows }) {
  ...
  if (activeLogin) {
    throw new Error('Stop the login flow before starting a run.');
  }
  const normalizedMode = mode === 'live' ? 'live' : 'dry-run';
  const normalizedMax = Number(maxUnfollows);
  validateRunLimit(normalizedMax, 'Max unfollows');

  const args = [
    'src/index.js',
    normalizedMode === 'live' ? '--live' : '--dry-run',
    ...(normalizedMode === 'live' ? ['--confirm-live'] : []),
    `--max-unfollows=${normalizedMax}`
  ];
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/config') {
    const body = await readJson(req);
    const account = validateInstagramUsername(body.account || '');
    const followingUrl = validateInstagramUrl(body.followingUrl || '');
    const cleanupMode = validateCleanupMode(body.cleanupMode || 'categories');
    const selectedCategories = validateCleanupCategories(
      Array.isArray(body.cleanupCategories) ? body.cleanupCategories : [],
      cleanupMode
    );
  ...
  if (req.method === 'POST' && pathname === '/api/scheduler') {
    ...
    validateRunLimit(maxUnfollows, 'Scheduled max unfollows');
    await updateEnvFile(envPath, {
      DAILY_MAX_UNFOLLOWS: String(maxUnfollows),
      DRY_RUN: body.live ? '0' : '1',
      CONFIRM_LIVE: body.live ? '1' : '0'
    });
