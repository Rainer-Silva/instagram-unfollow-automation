...
function confirmLiveRun({ account, maxUnfollows, cleanupMode, categories }) {
  return new Promise((resolve) => {
    const dialog = $('live-confirm-dialog');
    const details = $('live-confirm-details');
    details.innerHTML = '';
    const rows = [
      ['Instagram target username', account || '-'],
      ['Maximum unfollows', String(maxUnfollows)],
      ['Selected cleanup mode', cleanupMode],
      ['Selected categories', categories.length ? categories.join(', ') : 'none']
    ];
...
async function run(mode) {
  ...
  if (!Number.isInteger(maxUnfollows) || maxUnfollows < 0 || maxUnfollows > MAX_USER_UNFOLLOWS) {
    throw new Error(`Max unfollows must be an integer from 0 to ${MAX_USER_UNFOLLOWS}.`);
  }
  if (mode === 'live') {
    const cleanupMode = $('mode-all').checked ? 'all' : 'categories';
    const categories = [...document.querySelectorAll('#category-grid input:checked')]
      .map((input) => categoryLabels[input.value] || input.value);
    const account = $('target-account').value || lastStatus?.account || '-';
    const confirmed = await confirmLiveRun({ account, maxUnfollows, cleanupMode, categories });
    if (!confirmed) return;
  }
...
async function saveScheduler() {
  const maxUnfollows = Number($('schedule-max').value);
  if (!Number.isInteger(maxUnfollows) || maxUnfollows < 0 || maxUnfollows > MAX_USER_UNFOLLOWS) {
    throw new Error(`Scheduled daily cap must be an integer from 0 to ${MAX_USER_UNFOLLOWS}.`);
  }
